...
// 1024 (provider limit) - 26 chars (khulnasoft  internal suffixes) - 31 chars (blob `-randomId` suffix) = 967
...
export class BlobUnknownError extends BlobError {
  constructor() {
    super('Unknown error, please visit https://khulnasoft.com/help.');
  }
}
...
// The server (Khulnasoft Blob API) uses this information to change its behavior like the
// response format
...
function getApiVersion(): string {
  ...
  try {
    ...
    versionOverride =
      process.env.KHULNASOFT_BLOB_API_VERSION_OVERRIDE ||
      process.env.NEXT_PUBLIC_KHULNASOFT_BLOB_API_VERSION_OVERRIDE;
  } catch {
    // noop
  }
...
function getRetries(): number {
  try {
    const retries = process.env.KHULNASOFT_BLOB_RETRIES || '10';
...
function getProxyThroughAlternativeApiHeaderFromEnv(): {
  ...
  try {
    if (
      'KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API' in process.env &&
      process.env.KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API !== undefined
    ) {
      extraHeaders['x-proxy-through-alternative-api'] =
        process.env.KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API;
    } else if (
      'NEXT_PUBLIC_KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API' in
        process.env &&
      process.env.NEXT_PUBLIC_KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API !==
        undefined
    ) {
      extraHeaders['x-proxy-through-alternative-api'] =
        process.env.NEXT_PUBLIC_KHULNASOFT_BLOB_PROXY_THROUGH_ALTERNATIVE_API;
...
function shouldUseXContentLength(): boolean {
  try {
    return process.env.KHULNASOFT_BLOB_USE_X_CONTENT_LENGTH === '1';
  } catch {
    return false;
  }
}
