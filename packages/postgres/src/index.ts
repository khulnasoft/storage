...
import { type KhulnasoftPool, createPool } from './create-pool';
...
let pool: KhulnasoftPool | undefined;
...
export const sql = new Proxy(
  // eslint-disable-next-line @typescript-eslint/no-empty-function -- [@khulnasoft/style-guide@5 migration]
  () => {},
  {
    get(_, prop) {
      if (!pool) {
      ...
      if (typeof val === 'function') {
        // eslint-disable-next-line @typescript-eslint/no-unsafe-return, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call -- [@khulnasoft/style-guide@5 migration]
    ...
    apply(_, __, argumentsList) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-unsafe-argument -- [@khulnasoft/style-guide@5 migration]
      return pool.sql(...argumentsList);
    },
  },
) as unknown as KhulnasoftPool &
