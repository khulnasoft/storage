...
test.describe('@khulnasoft/postgres-kysely', () => {
    ...
    test.describe('api', () => {
      test('edge', async ({ request }) => {
        const res = await request.get(
          'api/khulnasoft/postgres-kysely/app/edge',
        );
        expect(await res.json()).toEqual(expectedRows);
      });
      test('node', async ({ request }) => {
        const res = await request.get(
          'api/khulnasoft/postgres-kysely/app/node',
        );
        expect(await res.json()).toEqual(expectedRows);
      });
    });
  ...
  test.describe('pages directory', () => {
    test.describe('api', () => {
      test('edge', async ({ request }) => {
        const res = await request.get(
          'api/khulnasoft/postgres-kysely/pages/edge',
        );
        expect(await res.json()).toEqual(expectedRows);
      });
      test('node', async ({ request }) => {
        const res = await request.get(
          'api/khulnasoft/postgres-kysely/pages/node',
        );
        expect(await res.json()).toEqual(expectedRows);
      });
    });
