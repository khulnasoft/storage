...
import type * as khulnasoftBlob from '@khulnasoft/blob';
import { useCallback, useEffect, useState } from 'react';
import { useSearchParams } from 'next/navigation';
import { API_ROOT } from '../../api/app/constants';

export default function AppList(): React.JSX.Element {
  const [result, setResult] = useState<khulnasoftBlob.ListBlobResult>();
  const [searchPrefix, setSearchPrefix] = useState('');
  const [urlsToRemove, setUrlsToRemove] = useState<string[]>([]);
  const [head, setHead] = useState<khulnasoftBlob.HeadBlobResult>();
  const searchParams = useSearchParams();
  const _access = searchParams?.get('access') ?? 'public';
  ...
  const getList = useCallback(
    async (
      ...
      const data = (await fetch(`${API_ROOT}/list?${search.toString()}`).then(
        (r) => r.json(),
      )) as khulnasoftBlob.ListBlobResult;
  ...
  const handleHead = async (url: string): Promise<void> => {
    ...
    }).then((r) => r.json())) as khulnasoftBlob.HeadBlobResult;
    setHead(data);
  };
