...
async function run(): Promise<void> {
  ...
  await Promise.all(urls.map((url) => khulnasoftBlob.del(url)));
}

async function textFileExample(): Promise<string> {
  const start = Date.now();
  const blob = await khulnasoftBlob.put('folderé/test.txt', 'Hello, world!', {
    access: 'public',
  });
...
async function videoExample(): Promise<string> {
  ...
  const blob = await khulnasoftBlob.put(pathname, createReadStream(fullPath), {
    access: 'public',
  });
...
async function noExtensionExample(): Promise<string> {
  const start = Date.now();
  const blob = await khulnasoftBlob.put('folder/test something', 'Hello, world!', {
    access: 'public',
  });
...
async function copyTextFile() {
  ...
  const copiedBlob = await khulnasoftBlob.copy(blob.url, 'destination/copy.txt', {
    access: 'public',
  });
...
async function multipartNodeJsFileStream() {
  ...
  const blob = await khulnasoftBlob.put(`éllo/${pathname}`, stream, {
    access: 'public',
    multipart: true,
  });
...
async function createFolder() {
  ...
  const blob = await khulnasoftBlob.put('foolder/', {
    access: 'public',
    addRandomSuffix: false,
  });
